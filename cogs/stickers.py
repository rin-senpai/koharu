...
class Stickers(commands.Cog, description='only took multiple years (I think?)'):
    ...
    @sticker.command(name='add', description='adds sticcccer')
    async def add(self, interaction: discord.Interaction, name: str, image: discord.Attachment = None, image_url: str = '', aliases: str = ''):
        ...
        await interaction.followup.send(f'Added {name}!\n\nAliases:\n\n{aliases_breakdown}', ephemeral=True)

    @sticker.command(name='send', description='Sends a sticker (who would have thought')
    async def send(self, interaction: discord.Interaction, name: str):
        name = name.replace('\n', ' ')
        sticker_id = await self.bot.db.fetchval('SELECT sticker_id FROM users_stickers WHERE user_id = $1 AND (name = $2 OR ((ARRAY_TO_STRING(aliases, $4) LIKE $3)))', interaction.user.id, name, '%' + name + '%', ';')
    ...
    @sticker.command(name='delete', description='Deletes a sticker or multiple (seperate with ":")')
    async def delete(self, interaction: discord.Interaction, names: str = ''):
        if names == '':
            ...
            for name in names_list:
                status = await self.bot.db.execute('DELETE FROM users_stickers WHERE user_id = $1 AND (name = $2 OR (ARRAY_TO_STRING(aliases, $4) LIKE $3))', interaction.user.id, name, '%' + name + '%', ';')
                if status != 'DELETE 0':
                    successful_deletes.append(name)
            await interaction.response.send_message(f'Murdered {", ".join(successful_deletes)}', ephemeral=True)

    @sticker.command(name='view', description='You view stickers (wow so cool)')
    ...
    @sticker.command(name='edit', description='You edit stickers now wow')
    ...
    async def steal(self, interaction: discord.Interaction, message: discord.Message):
        if message.webhook_id is not None and message.attachments == []:
            print('ez webook')
...
async def cleanup_name(name, db, user_id, prev_name = ''):
    ...
    if ':' in name:
        ...
        return (None, breakdown)
    if len(name) > 100:
        breakdown = 'Bruh max name length is 100...'
        return(None, breakdown)
    
    if prev_name != name:
        while await db.fetchval('SELECT name FROM users_stickers WHERE user_id = $1 AND (name = $2 OR (ARRAY_TO_STRING(aliases, $4) LIKE $3))', user_id, name,  '%' + name + '%', ';') is not None:
                if name[-2] == '~':
...
async def cleanup_aliases(aliases_list, db, user_id, prev_list=[]):
    ...
    for i in range(len(aliases_list)):
        if i == 16:
            if prev_list == []:
                breakdown += 'You can only have 15 aliases btw so some weren\'t added (one was automatically added)'
            else:
                breakdown += 'You can only have 15 aliases btw so some weren\'t added'
            break
        ...
        if aliases_list[i] != '' and aliases_list[i] not in cleaned_list:
            if aliases_list[i] not in prev_list:
                while await db.fetchval('SELECT name FROM users_stickers WHERE user_id = $1 AND (name = $2 OR (ARRAY_TO_STRING(aliases, $4) LIKE $3))', user_id, aliases_list[i], '%' + aliases_list[i] + '%', ';') is not None:
                    if aliases_list[i][-2] == '~':
            ...
            if ';' in aliases_list[i]:
                breakdown += aliases_list[i][:100] + '❎ NO SEMICOLONS!😾\n'
            elif ':' in aliases_list[i]:
                breakdown += aliases_list[i][:100] + '❎ NO COLONS!😾\n'
            elif len(aliases_list[i]) > 100:
                breakdown += aliases_list[i][:100] + '❎ THERE\'S NO WAY THAT THING WILL FIT!😾\n'
            else:
                cleaned_list.append(aliases_list[i])
                if aliases_list[i] not in prev_list:
                    breakdown += aliases_list[i] + '✅\n'
    
    return (cleaned_list, breakdown)
...
class DeleteSelect(ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message('I deleted the ones you selected ig', ephemeral=True)
        self.view.stop()
...
class StealButton(ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_modal(StealModal(sticker_id, name, self.parent.db))
...
class EditModal(ui.Modal, title='Edit Sticker'):
    def __init__(self, sticker_id, name, aliases, db, parent = None):
        ...
        self.aliases_input = ui.TextInput(label='Aliases', style = discord.TextStyle.long, required=False, min_length=0, max_length=100, default=aliases)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.parent is None:
            response = f'I edited {name} yay'
            if aliases_breakdown != '':
                response += f'\n\nAliases:\n\n{aliases_breakdown}'
...
class StealModal(ui.Modal, title='Steal Sticker'):
    def __init__(self, sticker_id, name, db):
        ...
        self.name_input = ui.TextInput(label='Name', style = discord.TextStyle.short, required=True, min_length=1, max_length=100, default=name)
        self.add_item(self.name_input)
        self.aliases_input = ui.TextInput(label='Aliases', style = discord.TextStyle.long, required=False, min_length=0, max_length=100)
        self.add_item(self.aliases_input)
