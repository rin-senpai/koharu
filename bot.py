...
from discord.ext.commands import AutoShardedBot, HelpCommand
import os
import traceback
import sys
...
class Bot(AutoShardedBot):
    ...
    async def on_message(self, msg):
        ...
        if msg.content.startswith('¥') and await self.is_owner(msg.author):
            msg.content = msg.content.replace('¥', 'senpai say -h ', 1)

        if msg.content.startswith('ø') and await self.is_owner(msg.author):
            msg.content = msg.content.replace('ø', 'senpai owoify -h ', 1)
